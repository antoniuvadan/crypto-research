#!/usr/bin/env python3
"""
CROSS-ASSET out-of-sample test: the frozen BTC candidate applied to ETHUSD_PERP.

This is the ETH sibling of ``backtest_oos_test.py``. The strategy is FROZEN exactly
as it was for BTC (Finding 8/9/10 -- NOTHING re-tuned, no ETH-specific fitting):
...
Only the ASSET BINDINGS differ from the BTC run:
  - data paths -> data/ETHUSD_PERP-*
  - contract notional -> $10/contract (ETHUSD_PERP COIN-M face value; BTC is $100).
    The per-trade bps edge is invariant to this (sizing is by dollar notional, fees
    are on notional); $10 only affects fill-sweep contract counts and dollar P&L.
  - bookTicker filename symbol -> ETHUSD_PERP

Reserved TEST period: 2024-06-25 -> 2024-10-14 (same window as the BTC one-shot).
For ETH nothing was ever "spent", but we deliberately mirror the BTC OOS window so
the two assets are directly comparable. Do NOT tune anything here to rescue the
result -- a null/negative on ETH is itself a valid cross-asset finding.

Warmup: data is loaded from 2024-04-15 so the trailing-7d event threshold and the
trailing-30d trim median are fully warmed for every test-period event. Only events
with decision_time in the test period are reported.

Usage:
  python strategies/backtest_oos_test_eth.py        # run from the repo root
"""
...
# --- ETH asset bindings (the ONLY differences from backtest_oos_test.py) --------
ETH_SYMBOL = "ETHUSD_PERP"
ETH_LIQ_PATH = Path("data/ETHUSD_PERP-liquidationSnapshot")
ETH_AGG_PATH = Path("data/ETHUSD_PERP-aggTrades")
ETH_BOOK_PATH = Path("data/ETHUSD_PERP-bookTicker")
CONTRACT = 10.0                          # $10/contract (BTC is $100)
# --------------------------------------------------------------------------------
...
OUT = Path("data/results/oos_test_eth_trades.csv")
...
def run_size(liq: pl.DataFrame, agg: pl.DataFrame, notional: float) -> pl.DataFrame:
    res = bm.run_liquidation_momentum_model_c_backtests(
        ...
        contract_notional_usd=CONTRACT,
        signal_direction_sign=-1, percentile=0.98, upper_percentile=None,
        book_ticker_dir=ETH_BOOK_PATH, book_ticker_symbol=ETH_SYMBOL,
...
def add_book_features(df: pl.DataFrame) -> pl.DataFrame:
    ...
    book = BookProvider(ETH_BOOK_PATH, symbol=ETH_SYMBOL)
    n = len(df)
...
def report(test_all: pl.DataFrame, notional_primary: float, test100: pl.DataFrame) -> None:
    ...
    print("\n" + "#" * 80)
    print("# CROSS-ASSET OUT-OF-SAMPLE TEST -- ETHUSD_PERP, 2024-06-25 -> 2024-10-14")
    print("# Frozen BTC strategy: trimmed 5-min >=98th taker tail reversion. NOTHING re-tuned.")
    print("#" * 80)
    ...
    print("\n=== 5. VERDICT vs BTC BASELINE ===")
    net_filt = _net_bps(kept, notional_primary)
    s = hac_stats(net_filt)
    print(f"  ETH OOS filtered net  {s['mean_bps']:+.2f} bps   t_NW {s['t_nw']:+.2f}")
    print( "  BTC OOS (F10)         +48.85 bps (t_NW 2.62), +39-43 floor -- same window")
    print( "  BTC within-train      +51.01 bps (t_NW 3.22) optimistic / +41-45 floor (F7/F9)")
    print( "  A confirmation extends the edge cross-asset; a null/negative is a valid")
    print( "  cross-asset finding -- do NOT re-tune on ETH to rescue it.")


def main() -> None:
    _eprint(f"loading ETH liq + agg (warmup {LOAD_START} -> test end {LOAD_END}) ...")
    liq = bt.load_liq_snap(ETH_LIQ_PATH, start_date=LOAD_START, end_date=LOAD_END).collect().sort("time_datetime")
    agg = bt.load_agg_trades(ETH_AGG_PATH, start_date=LOAD_START, end_date=LOAD_END).collect().sort("transact_time_datetime")
